...
function renderUI() {
  ...
  let displayList = MASTER_LIST.filter((item) => {
    ...
    else if (statusFilter === "researching")
      matchesStatus = item.current > 0 && item.current < item.required;
    else if (statusFilter === "incomplete")
      matchesStatus = item.current < item.required;
    ...
    .map((item) => {
      ...
      return `
            <tr>
                ...
                <td>
                  <img src="${assets.icon}" alt="" class="item-icon" onerror="this.src='icons/Default.png';">
                </td>
                <td>
                    <a href="${assets.wiki}" target="_blank" class="wiki-link">
                        <strong>${item.display == "Bone Block" ? "Bone Block (unobtainable)" : item.display}</strong>
                    </a>
                </td>
                <td>
                ...
                <td class="${isDone ? "done" : item.current === 0 ? "none" : "mid"}">
                    ${isDone ? "COMPLETE" : item.current === 0 ? "NOT STARTED" : "RESEARCHING"}
                </td>
            </tr>
        `;
    })
  ...
  document.getElementById("statProgressing").innerText = MASTER_LIST.filter(
